...
from training.metrics import base_metrics_class
...
class UIAViTDetector(AbstractDetector):
    ...
    def get_losses(self, data_dict: dict, pred_dict: dict) -> dict:
        ...
        if self.batch_cnt > self.batch_per_epoch and self.model.training:
            pcl_loss = self.loss_func["pcl"](pred_dict["attention_map_real"],
                                             ...
                                             data_dict["label"])
            eps = 1e-6
            regularization = self.loss_weight[1] * (
                1 / torch.clamp(torch.abs(self.model.c_real), min=eps) +
                1 / torch.clamp(torch.abs(self.model.c_fake), min=eps)
    ...
    def get_train_metrics(self, data_dict: dict, pred_dict: dict) -> dict:
        label = data_dict['label']
        pred = pred_dict['cls']
        auc, eer, acc, ap = base_metrics_class.calculate_metrics_for_train(label.detach(), pred.detach())
    ...
    def forward(self, data_dict: dict, inference=False) -> dict:
        ...
        if self.model.training and self.batch_cnt != 0 and self.batch_cnt % (self.config["batch_per_epoch"] // 2) == 0:
            if self.real_feature_list:
                real_feature_tensor = torch.cat(self.real_feature_list, dim=0).cuda()
                if torch.isfinite(real_feature_tensor).all():
                    self.real_inv_covariance = fit_inv_covariance(real_feature_tensor)
                    self.real_feature_mean = real_feature_tensor.mean(dim=0)
                else:
                    print("Warning: non-finite values in real features — skipping MVG update.")
                self.real_feature_list = []

            if self.fake_feature_list:
                fake_feature_tensor = torch.cat(self.fake_feature_list, dim=0).cuda()
                if torch.isfinite(fake_feature_tensor).all():
                    self.fake_inv_covariance = fit_inv_covariance(fake_feature_tensor)
                    self.fake_feature_mean = fake_feature_tensor.mean(dim=0)
                else:
                    print("Warning: non-finite values in fake features — skipping MVG update.")
        ...
        if len(real_indices) > 0:
            ...
            with torch.no_grad():
                self.real_feature_list.append(feature_patch_real.reshape(B * H * W, C))
        ...
        if len(fake_indices) > 0:
            ...
            with torch.no_grad():
                self.fake_feature_list.append(feature_patch_fake.reshape(B * H * W, C))


        if len(real_indices) > 0:
            attention_map_real = torch.sigmoid(torch.mean(attention_map[real_indices, :, 1:, 1:], dim=1))
        else:
            attention_map_real = torch.zeros((1, attention_map.size(-2)-1, attention_map.size(-1)-1), device=attention_map.device)
        
        if len(fake_indices) > 0:
            attention_map_fake = torch.sigmoid(torch.mean(attention_map[fake_indices, :, 1:, 1:], dim=1))
        else:
            attention_map_fake = torch.zeros((1, attention_map.size(-2)-1, attention_map.size(-1)-1), device=attention_map.device)
