...
class UCFDetector(AbstractDetector):
    ...
    def forward(self, data_dict: dict, inference=False) -> dict:
        # split the features into the content and forgery; if inference == true, we won't use labels
        ...
        if inference:
            ...
            return  pred_dict
            """
            # If you need to record probs internally, do it here—but skip label processing
            self.prob.append(prob_sha.detach().cpu().numpy())
            # Do not try to read or append data_dict["label"] here
