...
from training.metrics.base_metrics_class import calculate_metrics_for_train
...
class STILDetector(AbstractDetector):
    ...
    def features(self, data_dict: dict) -> torch.Tensor:
        ...
        # Collapse temporal+channel dims for STIL_Model input:
        inputs = data_dict["image"].view(bs, t * c, h, w)   # [B, T*C, H, W]

        # To get the per‐frame feature‐maps, run the backbone up to before its final FC:
        # 1) reshape into [B*T, C, H, W]
        x = inputs.view(bs * t, c, h, w)

        # 2) pass through base_model’s conv/bn/relu/maxpool:
        ...
        # 3) layer1 → layer4 (the ResNet “body”):
        x = base.layer1(x)   # → [B*T, 256, H/4, W/4]
        x = base.layer2(x)   # → [B*T, 512, H/8, W/8]
        x = base.layer3(x)   # → [B*T, 1024, H/16, W/16]
        x = base.layer4(x)   # → [B*T, 2048, H/32, W/32]

        # 4) reshape back to 5D [B, T, C', H', W']:
        _, c_out, h_out, w_out = x.shape
        feature_map = x.view(bs, t, c_out, h_out, w_out)
    ...
    def forward(self, data_dict: dict, inference=False) -> dict:
        bs, t, c, h, w = data_dict["image"].shape
        # Extract the 5D feature‐map:
        feature_map_5d = self.features(data_dict)       # [B, T, C', H', W']

        # Option A: collapse temporal dimension immediately:
        feature_map_4d = feature_map_5d.mean(dim=1)              # [B, C', H', W']
        #logits = self.model(data_dict["image"].view(bs, t*c, h, w)) 
        #          # existing code: runs STIL_Model→ final logits [B, num_class]

        # We’ll produce two things: the final logits and the raw map.
        inputs = data_dict["image"].view(bs, t*c, h, w)
        logits = self.model(inputs)   # [B, num_class]

        # Now build the prediction dict. Keep “feat” = feature_map_5d (you can collapse
        # to 4D here if the ensemble fuser expects per‐video maps; otherwise keep 5D).
...
class STIL_Model(nn.Module):
    ...
    def build_model(self):
        ...
        print("DEBUG: fc_feature_dim =", fc_feature_dim, type(fc_feature_dim))
        print("DEBUG: self.num_class   =", self.num_class,   type(self.num_class))
